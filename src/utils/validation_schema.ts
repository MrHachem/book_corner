    ...
    switch (validationArgument) {
      ...
      case "phone":
        validationSchema["phone"] = Yup.string()
          ...
          .matches(
            /^09\d{8}$/,
            "Phone number must be Syrian and start with 09"
          );

        break;
      case "lastname":
        validationSchema["lastname"] = Yup.string().required(
          "last name is required"
        );
        break;
      case "firstname":
        validationSchema["firstname"] = Yup.string().required(
          "firstname is required"
        );
        break;
      default:
        throw new Error(
          `Unsupported validation argument: ${validationArgument}`
        );
    }
  }
